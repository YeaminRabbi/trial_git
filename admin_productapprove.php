	...
	<!----------------------------------------------------->
		<div class="" style="padding-top: 20px;max-width: 90%;margin:auto;">
				<div style="padding-left: 41%">
					<h3>Product Request for Approval</h3>
				      ...
				      <?php
				       ...
				       foreach ($product as $key => $data): ?>
				      	...
				      	<tr>
					        ...
					        <td>
					        	<a class="btn btn-warning btn-productEdit">
					        		<i class="fa fa-eye" aria-hidden="true"></i>&nbspView
					        	</a>

					        	<a href="action.php?PCONFIRMATIONID=<?= $data['product_code'] ?>" class="btn btn-primary">
					        		<i class="fa fa-check-circle" aria-hidden="true"></i></i>&nbspConfirm
					        	</a>
			    ...
			    <div class="modal-content">
			      ...
			      <form action="action.php" method="POST" enctype="multipart/form-data">
				      ...
				      <div class="modal-footer">
				      	<button type="submit" class="btn btn-success" name="btn-vendorProductCONFIRM">Confirm</button>
...
<script>
	...
	$('.btn-productEdit').on('click',function(){
		...
		$('#quantity').val(data[3]);
		$('#category').val(data[4]);
		$('#parent_category').val(data[5]);
		$('#price').val(data[7]);
		$('#details').val(data[8]);	
		$('#product_img').val(data[2]);	
		$('#master_category').val(data[6]);	
...
<?php
    ...
    {
        $sql="Select * FROM products where approve_status=0";
